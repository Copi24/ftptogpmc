...
class PhotoOrganizer:
    ...
    def initialize_client(self) -> bool:
        ...
        try:
            ...
            # Get the actual cache path from the gpmc client
            # gpmc uses: ~/.gpmc/{email}/storage.db
    ...
    def organize_files(self, dry_run: bool = False, partial_albums: bool = True, 
        ...
        """
        logger.info("=" * 80)
        logger.info("📁 BUILDING FILE-TO-ALBUM MAPPING")
        logger.info("=" * 80)
        
        # Build the mapping from manifest structure
        self.build_file_to_album_map(self.manifest['structure'])
        
        logger.info(f"✅ Mapped {len(self.file_to_album_map)} files to albums")
        
        # Group files by album
        album_to_files = self.get_files_by_album()
        
        logger.info(f"📊 Files will be organized into {len(album_to_files)} albums")
        ...
        for album_name, filenames in sorted(album_to_files.items()):
            ...
            # Add media to album
            try:
                logger.info(f"   📤 Adding {len(media_keys)} files to album: {album_name}")
...
def main():
    ...
    logger.info(f"✅ Auth data found ({len(auth_data)} characters)")
    ...
    try:
        ...
        if failed > 0:
            logger.warning("⚠️ Some files failed to organize - check logs for details")
            sys.exit(1)
